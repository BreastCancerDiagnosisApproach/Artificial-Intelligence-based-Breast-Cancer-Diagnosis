...
class TCN:
    ...
    def __call__(self, inputs):
        ...
        x = inputs
        x1 = Convolution1D(self.nb_filters, 1, padding=self.padding, name=self.name + '_initial_conv')(x)
        x2 = Convolution1D(self.nb_filters, 1, padding=self.padding, name=self.name + '_initial_conv')(x1)
        x3 = Convolution1D(self.nb_filters, 1, padding=self.padding, name=self.name + '_initial_conv')(x2)
        x = (x1 + x2 + x3 / 3)
        skip_connections = []
...
def train_pred1(train_X, train_y, val_X, val_y, callback=None):
    epochs = 2
    for e in range(epochs):
        model = model_tcn(embed_size)
